...
class get_coupled_problem_matrix():
    ...
    IT IS NECESSARY TO READ THE NOTES ON THE NUMERICAL SCHEME IN ORDER TO UNDERSTAND THE MATRIX COMPOSITION. A LITTLE SUMMARY:
        
        
        STEADY STATE COUPLED SYSTEM GIVEN BY:   [a   b] * {   0  }   {      0      }
                                                [     ] * {      } = {             }
                                                [c   d] * {  q   }   {<\phi_vessel>}      
                                                
        a=\Theta=Lap_operator
        b => fluxes due to the boundary conditions, and hence, coupling
        c and d => equation for the coupling term (q) (since the intravascular concentration is fixed to <\phi_vessel>,
                                                       this lower part of the matrix is quite straight forward"""
...
def get_sol_coupled(Lap_operator,phi_vessel, Rv, inc_r, inc_s, in_boundary, K_effpiRvsquare, RHS_without_coupling):
    """GETS THE NUMERICAL SOLUTION OF THE COUPLED PROBLEM. IT NEEDS THE LAPLACIAN OPERATOR AND SOME GEOMETRY, 
    HENCE THE ONLY MODIFICATION THIS FUNCTION DOES IS TO INCLUDE THE LAPLACIAN TO INCLUDE THE COUPLED VESSEL.
    ...
    factor=1/(np.pi*(2*Rv+inc_r)*inc_r) #geometrical factor
    ...
    C2_boundary_factor=C0/(C1+1)
    
    print("C_2={}".format(C2_boundary_factor))
    print("C_1={}".format(C1))
    print("C_0={}".format(C0))
    
    #i=inn_boundary
    i=np.arange(len(phi_vessel))
    RHS[i]=-factor*C2_boundary_factor*phi_vessel
    
    #WATCH OUT FOR THIS LINE!!!!!
    Lap_operator[in_boundary, in_boundary]-=factor*C2_boundary_factor
...
def get_validation_q_imposed_outerDirichlet(q_vessel, R_max, Rv,r_points,s_points, L, D):
    """The Dirichlet north are set in the function
    
    FUNCTION USED AS VALIDATION. """
...
# =============================================================================
# SEVERAL TYPES OF INTERPOLATION FOR THE POST-PROCESSING AND SOME INTEGRALS
# =============================================================================
...
def interp_kernel_oneD(coarse,h_coarse, fine, h_fine):
    """THIS IS THE ONLY INTERPOLATION FUNCTION THAT IS USED DURING THE MATRIX ASSEMBLY OF THE PROBLEM 
    AND NOT JUST IN THE POST PROCESSING.
    IT IS NEEDED TO PROVIDE A "FINITE ELEMENT LIKE" INTERPOLATION OF THE VERY COARSE REGULAR TERM AT THE 
    VESSEL WALL IN ORDER TO APPROXIMATE THE FLUX. IT IS CHEAP AND INCREASES SUBSTANTIALLY THE ACCURACY"""
