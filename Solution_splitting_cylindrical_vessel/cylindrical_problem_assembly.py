...
def get_RHS(q_vessel,D, Rv, s, r, inc_s, inc_r,  east_boundary, west_boundary, ext_boundary, corners, der1, der2):
    """Sets the Dirichlet BC in the outer boundary and the Neumann BCs on the east and west boundaries, and F on everycell"""
    ext_boundary=np.concatenate([[corners[2]],ext_boundary,[corners[3]]])
# =============================================================================
#     der1=get_first_der(q_vessel, inc_s).dot(q_vessel)
#     der2=get_sec_der(q_vessel, inc_s).dot(q_vessel)
# =============================================================================
    
    RHS=np.array([])
    #First, we set F in the whole domain:
    for i in r:
        RHS=np.concatenate([RHS, -Green(i, Rv)*der2/D])
    
    for i in east_boundary:
        ri=r[i//len(s)]
        RHS[i]=-der1[-1]*Green(ri, Rv)/D
    for i in west_boundary:
        ri=r[i//len(s)]
        RHS[i]=der1[0]*Green(ri, Rv)/D     
    for i in ext_boundary:
        ri=r[i//len(s)]
        si=s[i%len(s)]
        pos_s=i%len(s)
        RHS[i]=-q_vessel[pos_s]*Green(ri, Rv)/D 
...
class simplified_assembly_Laplacian_cylindrical():
    """Functions necessary to assemble the laplacian operator"""
