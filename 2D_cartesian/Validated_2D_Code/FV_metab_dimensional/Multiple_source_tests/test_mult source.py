...
#sys.path.append('/home/pdavid//Bureau/Code/SS/2D_cartesian/SS_code/FV_metab_dimensional')
sys.path.append('..')
import numpy as np 
...
#a=2.1*Rv
#pos_s=np.array([[0.9,0.9],[0.75,0.75]])+L/2
#a=4*Rv
pos_s=np.array([[0.9,0.9],[0.62,0.62]])+L/2
#a=10*Rv
#pos_s=np.array([[0.9,0.9],[0.75,0.75]])+L/2
...
print("The exchanges for the coupling model are:", phi_q)
print("The sum:", np.sum(phi_q))
...
noc_sol, noc_lenx, noc_leny,noc_q, noc_B, noc_A, noc_s_blocks,noc_x,noc_y=ref_FV.sol, len(ref_FV.x), len(ref_FV.y), ref_FV.q_array, ref_FV.H0, ref_FV.A, ref_FV.s_blocks, ref_FV.x, ref_FV.y
...
p_sol, p_lenx, p_leny,p_q, p_B, p_A, p_s_blocks,p_x,p_y=ref_P.sol, len(ref_P.x), len(ref_P.y), ref_P.q_array, -ref_P.H0, ref_P.A, ref_P.s_blocks, ref_P.x, ref_P.y
