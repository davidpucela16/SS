...
def set_boundary_values(BC_type, BC_value, phi_FV, phi_q, x, y, boundary_vector,h,D, ass_object):
    ...
    for bc in BC_type:
        ...
        for k in boundary_vector[b,:]:
            ...
            if bc=='Neumann':
                s_km = phi_FV[k] - np.dot(r_k_grad_face_kernel, phi_q) - BC_value[c]*h/D
            if bc=='Periodic':
...
# =============================================================================
    ...
    def rec_corners(self):
        ...
        for i in range(4):
            ...
            self.rec_phi_FV=modify_matrix(self.rec_phi_FV, np.arange(len(x))[pos_x], np.arange(len(y))[pos_y], local_sol)
