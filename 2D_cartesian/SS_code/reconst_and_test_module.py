...
def reconst_real_fluxes_posteriori(full_solution, ass_object):
    ...
    for i in np.unique(t.s_blocks):
        grads[i]=manual_test_s_block_grads(t, phi, i)
        c=0
        for j in i+neigh:
...
def get_grad_singular(solution, x,y, block_ID, s_blocks, pos_s, h):
...
def manual_test_s_block_grads(ass_object, phi, block_ID):
    ...
    for i in range(4):
        ...
        grad_v[i]=grad
    Flux_v=-grad_v*D*t.h
    Flux_G=-get_grad_singular(phi, t.x,t.y, block_ID, t.s_blocks, t.pos_s, t.h)*D*t.h
    ...
    return(grad_v+get_grad_singular(phi, t.x,t.y, block_ID, t.s_blocks, t.pos_s, t.h))
