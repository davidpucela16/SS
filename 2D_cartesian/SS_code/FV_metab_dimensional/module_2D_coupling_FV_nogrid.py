...
#############################################################################################
                ...
                for j in other:
                    self.d_matrix[i,j]+=Green(self.pos_s[i],self.pos_s[j], self.Rv)/self.D
...
class FV_validation():
    ...
    def set_up_system(self):
        ...
        #set dirichlet
        H0,A=set_TPFA_Dirichlet(0,A, self.D, get_boundary_vector(len(x), len(y)), np.zeros(len(x)*len(y)))
        #Set sources
...
class non_linear_metab(assemble_SS_2D_FD):
    ...
    def reconstruct_field_kk(self, u, q):
    ...
    def assemble_it_matrices_Sampson(self, u, q):
        ...
        part_Im_u=np.zeros(self.phi_FV.size)  #\frac{\partial I_m}{\partial u}
        ...
        for k in range(self.phi_FV.size):
            ...
            for i in range(len(w_i)):
                ...
                S_bar=np.dot(kernel_G,q)
                rec_sing[k]+=w_i[i]*S_bar
                
                I_m[k]+=phi_0*w_i[i]/(phi_0+S_bar+u[k])
                part_Im_u[k]-=phi_0*w_i[i]/(phi_0+S_bar+u[k])**2
                for l in range(len(self.pos_s)):
                    #part_Im_q[k,l]+=part_Im_u[k]*kernel_G[l]
                    part_Im_q[k,l]-=kernel_G[l]*phi_0*w_i[i]/(phi_0+S_bar+u[k])**2
    
        self.I_m=I_m
        self.rec_sing=rec_sing
        self.part_Im_q=part_Im_q
        self.part_Im_u=part_Im_u
        return()
    ...
    def Full_Newton(self, u,q, rel_error,M, phi_0):
        ...
        while np.abs(rl[-1])>rel_error:
            ...
            Jacobian=np.concatenate((np.diag(self.part_Im_u)+self.A_matrix, 
