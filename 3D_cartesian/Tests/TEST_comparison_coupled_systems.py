...
init=np.array([0.2*Lx,0,0.45*Lz])
end=np.array([0.8*Lx,Ly,0.52*Lz])
...
get_25_profiles("x", np.array([len(op_FV.x),len(op_FV.y),len(op_FV.z)]), s.coup_cells, sol_FV,"FV")
...
o_FV=post_processing(sol_FV, op_FV, source)
...
o=post_processing(sol_FV, op_FV, s)
o.get_middle_plots(L,"FV")

get_profiles_comparison_25_plots("x", np.array([len(op_FV.x),len(op_FV.y),len(op_FV.z)]), sol_FV,np.array([len(t.x),len(t.y),len(t.z)]), sol_total, "ratio=2.5", op_FV.x, t.x)
